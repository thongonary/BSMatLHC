...
void TimeShower::init( BeamParticle* beamAPtrIn,
  ...
  */
  ...
  //Hideen Valley custom variables
  doHVRun = settingsPtr->flag("HiddenValley:Run");
  HiddenLambda = settingsPtr->parm("HiddenValley:Lambda");
  
  //nF, and nB for running
  nBFlavRun = settingsPtr->mode("HiddenValley:NBFlavRun");
  nFFlavRun = settingsPtr->mode("HiddenValley:NFFlavRun");
...
void TimeShower::pT2nextHV(double pT2begDip, double pT2sel,
  ...
  /*
  double run_coef = 
    (11./6. * 2 - 1.0/6.0*nBflav - 2.0/6.0*nFflav);
  */
  
  double myEmitCoef = colvFac * 2. * log(1. / zMinAbs - 1.);
  int Nc = settingsPtr->mode("HiddenValley:Ngauge");
  
  double HiddenLambda2 = HiddenLambda*HiddenLambda;

  //rescale max pt split to be above confinement
  if(doHVRun)
    pT2endDip = max(HiddenLambda2, pT2endDip);


  double run_coef = 
    (11./6. * Nc - nBFlavRun/6.0 - nFFlavRun/3.0);
  ...
  do {
    ...
    if(! doHVRun ) {
      ...
      dip.pT2 = HiddenLambda2*pow(dip.pT2/HiddenLambda2, 
	  pow(rndmPtr->flat(), run_coef/myEmitCoef));
      wt = 0.;
    ...
    // Abort evolution if below cutoff scale, or below another branching.
    if ( dip.pT2 < pT2endDip || dip.pT2 <= HiddenLambda2*1.01) { dip.pT2 = 0.; return; }
