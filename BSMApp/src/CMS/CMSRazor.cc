...
void CMSRazor::Loop(string outFileName) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry+=1) {
    ...
    */
    ...
    int numQuarks_1=0;
    int numQuarks_2=0;
    int threshold = 2; //how many quarks we should see: set at 4 for T1, 2 for ttbar
   ...
   vector<int> genParticleStatus;
	...
	if (!ttbar){
	  if (fabs(genParticlePdgId[p]) > 0 && fabs(genParticlePdgId[p]) < 9){ //if it is a quark
	    if (genParticleM1PdgId[p] == 1000021 || (genParticleM1PdgId[p] > 1000000 && genParticleM1PdgId[p] < 1000007) || (genParticleM1PdgId[p] > 2000000 && genParticleM1PdgId[p] < 2000007)) {// if mother is a gluino or a squark
